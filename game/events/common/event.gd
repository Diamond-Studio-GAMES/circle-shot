...
func _ready() -> void:
	...
	for i: String in Globals.items_db.spawnable_projectiles:
	...
	for i: String in Globals.items_db.spawnable_other:
...
func spawn_player(id: int) -> void:
	...
	var player_scene: PackedScene = load(Globals.items_db.skins[data[1]].skin_path)
...
func _on_player_killed(who: int, by: int) -> void:
	var message_text: String = "[color=#%s]%s[/color] убивает игрока [color=#%s]%s[/color]!" % [
		Entity.TEAM_COLORS[_players_data[by][6]].to_html(false),
		_players_data[by][0],
		Entity.TEAM_COLORS[_players_data[who][6]].to_html(false),
		_players_data[who][0],
	]
...
func _on_peer_disconnected(id: int) -> void:
	var message_text: String = "Игрок [color=#%s]%s[/color] отключился!" % [
		Entity.TEAM_COLORS[_players_data[id][6]].to_html(false),
		_players_data[id][0],
	]
