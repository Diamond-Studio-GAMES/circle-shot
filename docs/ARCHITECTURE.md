...
## Файловая структура
 ...
 - Папка `game`: содержит всё, что связано с самой игрой. Это:
    ...
    - Папка `menus`: содержит различные меню, такие как:
        - Папка `connect`: содержит меню для подключения по локальной сети и к онлайн-играм (будет реализовано позже).
    ...
    - Папка `ui`: ресурсы интерфейса, характерные только для игровой части.
 ...
 - Папка `menu`: содержит в себе главное меню, магазин и прочие штуки, не связанные с игрой. Точная структура будет доработана позже.
    - Папка `main`: содержит ресурсы для главного меню игры.
